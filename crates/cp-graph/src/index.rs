...
//!
//! Per CP-002: Provides mmap-backed HNSW for fast vector search
...
impl PersistentHnswIndex {
    ...
    pub fn new(config: IndexConfig) -> Result<Self> {
        ...
        let index = Index::new(&options)
            .map_err(|e| CPError::Database(format!("Failed to create index: {}", e)))?;

        index
            .reserve(config.capacity)
            .map_err(|e| CPError::Database(format!("Failed to reserve capacity: {}", e)))?;
    ...
    fn load(&mut self) -> Result<()> {
        if let Some(path) = self.index_path.clone() {
            self.index
                ...
                .map_err(|e| CPError::Database(format!("Failed to load index: {}", e)))?;

            // Load mapping file
            let map_path = path.with_extension("map");
            if map_path.exists() {
                let data = std::fs::read(&map_path)
                    .map_err(|e| CPError::Database(format!("Failed to read map: {}", e)))?;
            ...
            if checkpoint_path.exists() {
                let data = std::fs::read(&checkpoint_path)
                    .map_err(|e| CPError::Database(format!("Failed to read checkpoint: {}", e)))?;
                if data.len() == 32 {
                    let mut root = [0u8; 32];
    ...
    pub fn save(&self) -> Result<()> {
        if let Some(path) = &self.index_path {
            self.index
                ...
                .map_err(|e| CPError::Database(format!("Failed to save index: {}", e)))?;

            // Save mapping
            let map_path = path.with_extension("map");
            let map_data = self.serialize_mapping();
            std::fs::write(&map_path, map_data)
                .map_err(|e| CPError::Database(format!("Failed to write map: {}", e)))?;

            // Save checkpoint
            if let Some(root) = &self.checkpoint_root {
                let checkpoint_path = path.with_extension("checkpoint");
                std::fs::write(&checkpoint_path, root)
                    .map_err(|e| CPError::Database(format!("Failed to write checkpoint: {}", e)))?;
            }
    ...
    fn load_mapping(&mut self, data: &[u8]) -> Result<()> {
        if data.len() < 16 {
            return Err(CPError::Database("Invalid mapping data".into()));
        }
        ...
        for _ in 0..count {
            if offset + 24 > data.len() {
                return Err(CPError::Database("Truncated mapping data".into()));
            }
            let uuid = Uuid::from_slice(&data[offset..offset + 16])
                .map_err(|_| CPError::Database("Invalid UUID in mapping".into()))?;
    ...
    pub fn insert(&mut self, emb_id: Uuid, vector: Vec<f32>) -> Result<()> {
        ...
        self.index
            .add(key, &vector)
            .map_err(|e| CPError::Database(format!("Failed to add vector: {}", e)))?;
    ...
    pub fn clear(&mut self) -> Result<()> {
        ...
        self.index = Index::new(&options)
            .map_err(|e| CPError::Database(format!("Failed to recreate index: {}", e)))?;
