# CP-003: Determinism & Math

> **Spec Version**: 1.0.0
> **Author**: Nadeem Bhati
> **Category**: Core
> **Requires**: CP-001, CP-002

## Synopsis

This specification defines the mechanisms by which CP achieves **true determinism**—the property that identical inputs always produce identical outputs across all devices, platforms, and time. This is the foundational guarantee that enables trustless verification of the semantic substrate.

## Motivation

Determinism is the **core invariant** upon which all CP guarantees depend:
...
CP solves these problems through a combination of:
...
**Definition**: CP is deterministic if and only if:
...
All data in CP MUST have a single, unambiguous binary representation.
...
```
function canonicalize_text(input: &str) -> Vec<u8>:
    // 1. Convert to owned String if needed
    let text = input.to_string()

    // 2. Unicode NFC normalization
    // Use: unicode-normalization crate (Rust) or java.text.Normalizer (Java)
    ...
    // 4. Split into lines, trim trailing whitespace
    lines = text.split('\n')
    trimmed_lines = lines.map(|line| line.trim_end())
    text = trimmed_lines.join("\n")
...
**Applied To**:
- Document content (before content_hash)
- Chunk text (before text_hash)
- Path strings (before path_id)

#### 2.2 Numerical Representation (Guidance)

While CP strongly recommends **Integer-only** or **SoftFloat** arithmetic for maximum auditability, this is **NOT** a protocol requirement for V1.
...
struct ModelManifest {
    model_id: String,                    // e.g., "cp/minilm-v1"
...
// OR
model_url = f"https://models.cp.dev/v1/{manifest.manifest_hash}"
```
...
CP uses **Hybrid Logical Clocks (HLC)** instead of wall clocks.
...
CP uses **Last-Writer-Wins (LWW)** based on Hybrid Logical Clocks (HLC).
...
CP guarantees that for a given query, **every device will assemble the EXACT same context**.
...
This pivot acknowledges that while we can control the *library* (CP), we cannot strictly control the *oracle* (The LLM) without massive performance penalties. We choose to make the **Substrate** deterministic, and the **Intelligence** accountable (via Context Hashing).
